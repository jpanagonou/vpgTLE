...
arr_orig = itk.array_from_image(itk.imread('output/stage1a/VPG_vpg_database_Gate10_e.nii.gz'))
arr_opti = itk.array_from_image(itk.imread('output/stage1a/VPG_vpg_database_Gate9_e.nii.gz'))
...
def plot_spectre(z_idx, y_idx, x_idx, label, filename):
    ...
    # Spectres superposés
    axes[0].plot(E, spectre_orig, linewidth=1.5,   label='database Gate 10', color='blue')
    axes[0].plot(E, spectre_opti, linewidth=1, label='database Gate 9', color='red', linestyle='--')
    axes[0].set_xlabel("Énergie des PG (MeV)")
    axes[0].set_ylabel("PG yields / primary")
    axes[0].set_title(f'Spectres des PG dans un voxel du {label} : Gate 10/Gate 9')
    axes[0].legend()
