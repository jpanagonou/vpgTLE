...
with uproot.recreate(output_dir / "PGdb_gate9.root") as out:
    for element, symbol in symbols.items():
        fname = output_dir / f"db-{element}.root"
        if fname.exists():
            f = uproot.open(fname)
            # Les histogrammes sont dans le sous-répertoire element/
            subdir = f[element]
