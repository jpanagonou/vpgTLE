...
class tle_sim:
    ...
    def find_emission_vector(
        ...
        #if el == "Phosphor":
        #    el = "Phosphorus"
        #el = elements_name_symbol.get(el, elements_name_symbol.get(el.capitalize(), el))
        el = elements_name_symbol[el]
        #el = elements_name_symbol.get(el, el)
    ...
    def density_mat(
        self, mat, data_way
    ):  # read the datas to find the density of the material
    ...
    def voxel_to_mat_name(self, UH, mat_data):  # from UH to material name
        ind = 0
        for l in mat_data:
            ind = ind + 1
            if ind == len(mat_data):
    ...
    def mat_to_UH(self, mat, mat_data):  # from material to UH
        mat = mat.lower()
        for l in mat_data:
            print(l[2])
    ...
    def gamma_mat(self, name, root_data):
        Gamma = np.zeros((500, 250))
        rho_mat = self.density_mat(name, self.data_way)

        # Obtenir le nom de base sans suffixe _01, _02...
        ...
        # Vérifier le cache
        cache_key = (base_name, id(root_data))
        if cache_key in self._gamma_cache:
            return self._gamma_cache[cache_key] * rho_mat

        # Calculer pour la première fois
        (elements, fractions) = self.liste_el_frac(name, self.mat_fraction)

        """
        vects = np.array([self.find_emission_vector(el, root_data) for el in elements])  # (N_el, 500, 250)
        weights = np.array([fractions[i] / 100 for i in range(len(elements))])  # (N_el,)
        Gamma_base = np.einsum('i,ijk->jk', weights, vects)  # (500, 250)
        """

        
        Gamma_base = np.zeros((500, 250))
        for el in elements:
        ...
        self._gamma_cache[cache_key] = Gamma_base

        return Gamma_base * rho_mat
