...
// --- AJOUT ---
// Remplace le generateur d'energie par une variante specifique (ex : PG
// voxelisee). Suit le meme principe que SetPosGenerator ci-dessus : on ne
// detruit pas l'ancien generateur ici (coherent avec le choix deja fait
// pour SetPosGenerator), c'est le destructeur qui s'en charge globalement.
void GateSingleParticleSource::SetEneGenerator(GateSPSEneDistribution *eg) {
 fEnergyGenerator = eg;
 fEnergyGenerator->SetBiasRndm(fBiasRndm);
}
// ---------------------------------------------------
