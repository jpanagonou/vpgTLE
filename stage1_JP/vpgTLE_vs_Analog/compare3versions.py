...
def plot_spectre(z_idx, y_idx, x_idx, label, filename):
    ...
    # ── Écart relatif sans masque ─────────────────────────────────────
    ecart_viol = np.abs(spectre_analog - spectre_violette) / (spectre_analog + 1e-30) * 100
    ecart_jp   = np.abs(spectre_analog - spectre_jp)       / (spectre_analog + 1e-30) * 100
    ...
    print(f"  Écart Analog vs Violette — moyen : {ecart_viol.mean():.2e} %  max : {ecart_viol.max():.2e} %")
    print(f"  Écart Analog vs JP       — moyen : {ecart_jp.mean():.2e} %  max : {ecart_jp.max():.2e} %")
