...
"""
Test minimal fonctionnel de VoxelizedPromptGammaTLESource.
...
if __name__ == "__main__":
   ...
   # ecriture sur disque (necessaire car VoxelizedPromptGammaTLESource lit
   # un fichier, comme VoxelSource)
   ...
   # volume englobant toute l'image (nécessaire pour attacher la source
   # ET le PhaseSpaceActor ; le volume doit être assez grand pour contenir
   ...
   # source voxelisée PG, attachée à ce volume
   ...
   # cuts / physique minimale (pas de transport complexe nécessaire ici,
   # on veut juste vérifier le tirage à la génération)
   ...
   # déplacement avant), KineticEnergy = énergie initiale tirée par la CDF.
   ...
   # ------------------------------------------------------------------
   # vérification : position et énergie de chaque particule générée
   # ------------------------------------------------------------------
   import uproot
   ...
   print(f"\nNombre de particules enregistrées : {len(energies)}")


   # vérification énergie : toutes proches de E_test (tolérance = largeur
   ...
   # vérification position : toutes dans le voxel actif
   # (coordonnées centrées sur l'image, voxel [ix,iy,iz] au centre ici)
   voxel_half = spacing_mm / 2.0
   # position attendue du centre du voxel actif (image centrée en 0)
